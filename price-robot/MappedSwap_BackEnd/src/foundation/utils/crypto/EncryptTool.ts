...
export default class EncryptTool {
  ...
  async start() {
    ...
    while (!allowedMode.includes(encryptMode)) {
      // encryptMode = await this.readlineHelper.ask(
  ...
  async encryptByPublicKey() {
    ...
    while (true) {
      this.publicKey = await this.readlineHelper.ask(
        'Please enter your public key: ',
      );
      console.log(`Public key : ${this.publicKey}`);
      yesOrNo = await this.readlineHelper.ask(
    ...
    // eslint-disable-next-line no-constant-condition
    while (true) {
      const message = await this.readlineHelper.ask(
  ...
  async decryptByPrivateKey() {
    ...
    // eslint-disable-next-line no-constant-condition
    while (true) {
      this.privateKey = await this.readlineHelper.ask(
        'Please enter your private key: ',
      );
    ...
    while (message.toLowerCase() != 'exit') {
      message = await this.readlineHelper.ask(
