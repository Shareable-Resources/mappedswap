...
export function amountDivideDecimals(amount, decimals) {
  ...
  try {
    ...
    console.log('amountDivideDecimals error:', err)
  }
  return output
}

export function amountDivideDecimalsToDisplay(amount, decimals) {
  let output = amountDivideDecimals(amount, decimals)
  try {
    output = numberToFormattedString(output, decimals)
  } catch (err) {
    // amountDivideDecimals error
    console.log('amountDivideDecimals error:', err)
  }
...
export function getDecimalsByToken(tokenName) {
  let decimals = 0
  if (tokenName === 'USDL') {
    decimals = 6
  } else if (tokenName === 'BTCL') {
    decimals = 8
  } else if (tokenName === 'ETHL') {
    decimals = 18
  }
  return decimals
}

export function web3Sha3(inputStr) {
  const hash = sha3(inputStr)
  return hash
}
