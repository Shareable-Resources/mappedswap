...
contract MappedSwapToken is OwnableUpgradeable, AccessControlUpgradeable, ERC20Upgradeable, IMappedSwapToken, IEurusERC20 {
    ...
    function submitWithdraw(
        ...
        require(walletAddrMap.isWalletAddressExist(_msgSender()), "Wallet is not registered");

        string memory isDecentralizedUser = walletAddrMap.getWalletInfoValue(_msgSender(), "isMetaMask");
        string memory kycLevel;
        if (keccak256(abi.encodePacked(isDecentralizedUser)) == keccak256(abi.encodePacked("true"))) {
            //Decentralized user must use KYC level 1
            kycLevel = "1";
        } else {
            kycLevel = walletAddrMap.getWalletInfoValue(_msgSender(), "kycLevel");
        }
        ...
        if ((int256(block.timestamp) / int256(86400)) * int256(86400) == (int256(walletAddrMap.getLastUpdateTime(_msgSender())) / int256(86400)) * int256(86400)) {
            require(externalSCConfig.getCurrencyKycLimit(symbol(), kycLevel) >= dailyWithdrewAmount[_msgSender()] + withdrawAmount, "exceed daily withdraw amount");
            dailyWithdrewAmount[_msgSender()] = dailyWithdrewAmount[_msgSender()] + withdrawAmount;
        } else {
            require(externalSCConfig.getCurrencyKycLimit(symbol(), kycLevel) >= withdrawAmount, "exceed daily withdraw amount");
        ...
        bool isSuccess = transfer(adminFeeWalletAddr, feeAmount);
        require(isSuccess, "Transfer to AdminFee Wallet failed");

        address approvalWalletAddr = internalSCConfig.getApprovalWalletAddress();
        isSuccess = transfer(approvalWalletAddr, withdrawAmount);
