...
contract PoolCore is OwnableUpgradeable, AccessControlUpgradeable, ERC165Upgradeable, IERC165, IERC223Recipient, IPoolCustomerInfo, IPoolInternal, IPoolStopout, IPoolTrading {
    ...
    IHashedKeyStorage private tradeRecordContract;
    ...
    function withdraw(string calldata tokenName, uint256 amount) external override nonReentrant {
        _withdraw(tokenName, amount);

        SafeERC20.safeTransfer(tokens[tokenName].tokenAddr, msg.sender, amount);
    }

    function withdrawAndExchange(
        ...
        IERC20 fromToken = tokens[tokenName].tokenAddr;
        SafeERC20.safeApprove(fromToken, address(exchange), amount);
        exchange.exchangeFromExactAmount(provider, address(fromToken), address(toToken), amount, amountOutMin, msg.sender);
        directFundingToPool = Constant.NOT_ENTERED;
    }

    function _withdraw(string memory tokenName, uint256 amount) private {
        customerNotDisabled(msg.sender);
        PoolUtilities.tokenMustExist(tokenList, tokenName);

        PoolStruct.Customer storage c = customers[msg.sender];

        // Ensure not using any funding
        settleInterest(msg.sender);
        (uint256 usedFunding, ) = getUsedFundingAndEquity(msg.sender);
        ...
        emit Withdraw(msg.sender, tokenName, amount, newBalance);
    }
    ...
    function buy(
        ...
        uint256 amountSell;
        {
            ...
            writeTradeRecord(customer, amountSell, amountBuy, tokenNameSell, tokenNameBuy);
        }
    ...
    function stopout(address customer) external override onlyBackend {
        ...
        for (uint256 i = 0; i < tokenList.length; i++) {
            ...
            if (displayBalance == 0) {
                ...
                writeTradeRecord(customer, amountSell, amountBuy, tokenName, Constant.REF_TOKEN_NAME);
                ...
                writeTradeRecord(customer, amountSell, amountBuy, Constant.REF_TOKEN_NAME, tokenName);
    ...
    function writeTradeRecord(
        ...
        IHashedKeyStorage c = tradeRecordContract;

        // Prevent revert if contract address is still not set
        if (address(c) == address(0)) {
            return;
        }

        // Only care the hashed key, so data part can just be some simple text
        // Use name instead of address to hash, because this can be easily to call buy/sell after verified the hash
        bytes32 hashedKey = keccak256(abi.encodePacked(uint256(block.number), customer, amountSell, amountBuy, tokenNameSell, tokenNameBuy));
        c.insertData(hashedKey, "1");
    }
    ...
    function getTradeRecordContract() external view override returns (address) {
        return address(tradeRecordContract);
    }

    function setTradeRecordContract(address newTradeRecordContract) external override onlyOwner {
        tradeRecordContract = IHashedKeyStorage(newTradeRecordContract);
    }
