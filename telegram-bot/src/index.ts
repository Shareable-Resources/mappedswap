...
function main(bot = defaultBot, logs = true) {
  ...
  bot.use((ctx, next) => {
    ...
    if (cache.config.dev_mode) {
      middleware.reply(ctx, '<i>Dev Mode was on. turn if off.</i>', Extra.HTML());
  ...
  bot.command('start', (ctx) => {
      ...
      if (ctx.chat.type == 'private') {
        middleware.reply(ctx, cache.config.language.languageSelection, cache.config.menus.langMenu);
