...
import AgentServerConfig from './model/AgentServerConfig';
...
async function loadConfig() {
  ...
  //Read Config First
  const readServerFileResult = await readFile(ConfigName.Agent);
  const serviceConfig: AgentServerConfig = readServerFileResult.data;
  ...
  if (env) {
    globalVar.agentConfig = serviceConfig;
    ...
    logger.default.info(
      `Starting ${globalVar.agentConfig.name} Service with env [${env}]`,
    );
  } else {
