...
import Service from '../service/InterestService';
...
export class Controller implements CommonController {
  ...
  /**
   * Filter the list of interest based on
     [Required, from jwt] : agentId(jwt.id)
     [Required, from req] : recordPerPage, pageNo
     [Optional, from req] : fromTime,toTime,token,address,name
   */
  ...
  async getTotalInterest(req: any, res: any) {
    try {
    ...
    const data = await this.service.getTotalInterest(req.query);
    return apiResponse(
      res,
      responseBySuccess(data, true, 'query', 'success', 'Founded'),
      statusCode(true, 'query'),
