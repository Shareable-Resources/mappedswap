...
import Service from '../service/CustomerCreditUpdateService';
...
export class Controller implements CommonController {
  ...
  constructor(
    ...
    this.getAllWithCustomerInfo = this.getAllWithCustomerInfo.bind(this);
  ...
  async getAllWithCustomerInfo(req: any, res: any) {
    try {
      this.val.isNullOrEmpty(req.query.recordPerPage, 'recordPerPage');
      this.val.isNum(req.query.recordPerPage, 'recordPerPage');
      this.val.isNullOrEmpty(req.query.pageNo, 'pageNo');
    ...
    const data = await this.service.getAllWithCustomerInfo(
      req.query,
      req.jwt.id,
    );
    return apiResponse(
