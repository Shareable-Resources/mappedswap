...
logger.info(
  `Express server [${globalVar.agentConfig.name}] started on port ${globalVar.agentConfig.express.port}. Try some routes, such as '/api/users'.`,
);
...
export const printRoutes = (server: express.Express) => {
  ...
  routes.forEach((r) => {
    ...
    logger.info(
      `localhost:${globalVar.agentConfig.express.port}${
        r.path
      }  (${methods.join(',')})`,
    );
  });
