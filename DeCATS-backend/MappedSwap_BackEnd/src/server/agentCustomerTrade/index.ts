...
import ProfitDailyReportConfig from './model/EventServerConfig';
...
async function loadConfig() {
  ...
  //Read Config First
  const readServerFileResult = await readFile(ConfigName.Event);
  ...
  if (env) {
    globalVar.eventConfig = serviceConfig;
    ...
    logger.default.info(
      `Starting ${globalVar.eventConfig.name} Service with env [${env}]`,
    );
  } else {
