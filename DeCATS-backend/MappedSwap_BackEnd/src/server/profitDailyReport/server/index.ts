...
logger.info(
  `Express server [${globalVar.profitDailyReportConfig.name}] started on port ${globalVar.profitDailyReportConfig.express.port}. Try some routes, such as '/api/users'.`,
);
...
export const printRoutes = (server: express.Express) => {
  ...
  routes.forEach((r) => {
    ...
    logger.info(
      `localhost:${globalVar.profitDailyReportConfig.express.port}${
        r.path
      }  (${methods.join(',')})`,
    );
  });
