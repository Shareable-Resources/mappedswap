...
logger.info(
  `Express server [${globalVar.eventConfig.name}] started on port ${globalVar.eventConfig.express.port}. Try some routes, such as '/api/users'.`,
);
...
export const printRoutes = (server: express.Express) => {
  ...
  routes.forEach((r) => {
    ...
    logger.info(
      `localhost:${globalVar.eventConfig.express.port}${
        r.path
      }  (${methods.join(',')})`,
    );
  });
