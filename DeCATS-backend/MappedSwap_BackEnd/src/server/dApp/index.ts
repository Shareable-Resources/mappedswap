...
import DAppServerConfig from './model/DAppServerConfig';
...
async function loadConfig() {
  ...
  //Read Config First
  const readServerFileResult = await readFile(ConfigName.DApp);
  const serviceConfig: DAppServerConfig = readServerFileResult.data;
  ...
  if (env) {
    globalVar.dAppConfig = serviceConfig;
    ...
    logger.default.info(
      `Starting ${globalVar.dAppConfig.name} Service with env [${env}]`,
    );
  } else {
