...
logger.info(
  `Express server [${globalVar.dAppConfig.name}] started on port ${globalVar.dAppConfig.express.port}. Try some routes, such as '/api/users'.`,
);
...
export const printRoutes = (server: express.Express) => {
  ...
  routes.forEach((r) => {
    ...
    logger.info(
      `localhost:${globalVar.dAppConfig.express.port}${r.path}  (${methods.join(
        ',',
      )})`,
    );
  });
