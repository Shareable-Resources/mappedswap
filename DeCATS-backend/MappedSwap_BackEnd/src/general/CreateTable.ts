...
async function loadConfig() {
  ...
  //Read Config First
  const readServerFileResult = await readFile(ConfigName.MappedSwap);
  const serviceConfig: DecatsConfig = readServerFileResult.data;
  ...
  if (env) {
    globalVar.decatsConfig = serviceConfig;
    ...
    logger.default.info(
      `Starting ${globalVar.decatsConfig.name} Service with env [${env}]`,
    );
  } else {
