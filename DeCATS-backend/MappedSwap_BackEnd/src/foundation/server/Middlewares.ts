...
import {
  FundingCodeEncryptedObject,
  TokenEncryptedObject,
  TokenEncryptedObjectAgent,
  TokenEncryptedObjectCustomer,
} from '../types/TokenEncryptedObject';
...
export async function signToken(
  ...
  const TOKEN_KEY = globalVar.foundationConfig.jwt.TOKEN_KEY;
  let expiresIn = globalVar.foundationConfig.jwt.expires_in;
  if (obj.type == 'Customer') {
    expiresIn = globalVar.foundationConfig.jwt.dapp_expires_in;
  }
  // if (obj.type == 'FundingCode') {
  //   TOKEN_KEY = config.fundingCode.TOKEN_KEY;
  //   expiresIn = config.fundingCode.expires_in;
  // }

  return jwt.sign(obj, TOKEN_KEY, {
    expiresIn: expiresIn,
  });
}
...
export function validateAgentType(
  ...
  return async function (req, res, next) {
    try {
      if (req.jwt.agentType != agentType) {
        throw Error(
          `Incorrect Agent Type (Currect agent type:${req.jwt.agentType}). Expected agent type (${agentType})`,
        );
      } else {
        next();
      }
...
export function validateToken(logger: winston.Logger) {
  return async function (req, res, next) {
    try {
      const TOKEN_KEY = globalVar.foundationConfig.jwt.TOKEN_KEY;
      const decode = await jwt.verify(req.headers['authorization'], TOKEN_KEY);
      req.jwt = decode;
      next();
    } catch (e: any) {
      logger.error(e);
...
export async function signFundingCode(
  obj: FundingCodeEncryptedObject,
  expiry: any,
) {
  const TOKEN_KEY = globalVar.foundationConfig.fundingCode.TOKEN_KEY;
  // const expiresIn = config.fundingCode.expires_in;
...
export async function validateFundingCode(
  ...
  await jwt.verify(
    fundingCode,
    globalVar.foundationConfig.fundingCode.TOKEN_KEY,
