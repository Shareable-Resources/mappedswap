...
serviceMap[5]="profitDailyReport"
serviceMap[6]="event"
# -------------------
webpackFromNodeModules() {
  for s in "${!serviceMap[@]}"; do
    cmdStr="${cmdPath} --config ${srcPath}/${serviceMap[$s]}/${serviceMap[$s]}.${1}.js"
    echo ${cmdStr}
    ./node_modules/.bin/webpack --config "${srcPath}/${serviceMap[$s]}/${serviceMap[$s]}.${1}.js"
  done
}
join_by() {
  local d=${1-} f=${2-}
