...
import TradeInterestMobileRecord from "./TradeInterestMobileRecord";
...
const TradeOrderMobileRecord = () => {
  ...
  return (
    <>
      {data && data[0] ? (
        <InfiniteScroll dataLength={pageSize} next={getNextList} scrollThreshold={0.75} hasMore={hasMore} scrollableTarget="scrollableMobileInterest">
          <div className={style["table-wrapper"]}>
            <table>
              ...
              <tbody>
                {data.map((element) => {
                  return <TradeInterestMobileRecord key={element.id} id={element.id} createdDate={element.createdDate} token={element.token} interest={element.interest} rate={element.rate} address={element.address} status={element.status} amount={element.amount} />;
                })}
              </tbody>
            </table>
          </div>
