...
function deriveFromPassword(password, salt, rounds) {
    ...
    let derivedKeyData = pbkdf2(password, salt, rounds, bits);
    let derivedKeyHex = derivedKeyData.toString("hex");
...
function encryptText(text, password) {
    ...
    let iv = generateIV();
    let derivedKey = deriveFromPassword(password, salt, DERIVATION_ROUNDS);
...
function decryptText(encryptedString, password) {
    ...
    var derivedKey = deriveFromPassword(password, encryptedComponents.s, encryptedComponents.r);
...
function packageComponents(encryptedContent, components) {
    return `$mappedswap$${Object.keys(components).map(key => `${key}=${components[key]}`).join(",")}$${encryptedContent}`;
}
...
function privateDecrypt(privateKey, enc) {
     return crypto.privateDecrypt({key:privateKey, padding:crypto.constants.RSA_PKCS1_PADDING}, Buffer.from(enc, "base64")).toString();
}
function publicEncrypt(publicKey, dec) {
     return crypto.publicEncrypt({key:publicKey, padding:crypto.constants.RSA_PKCS1_PADDING}, Buffer.from(dec)).toString("base64");
}
